...
function showDashboard(adminData) {
    ...
    // Personalize
    const name = adminData.full_name || (adminData.email ? adminData.email.split('@')[0] : 'Admin');
...
function closeOpportunityModal() {
    ...
    setTimeout(() => { editingOpportunityId = null; }, 300);
}
...
document.getElementById('opportunityForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (response.ok) {
            showToast(`Opportunity ${editingOpportunityId ? 'updated' : 'created'} successfully!`);
            closeOpportunityModal();
            this.reset();
        ...
        data.opportunities.forEach(opp => {
            ...
            card.innerHTML = `
                ...
                <div class="opportunity-footer">
                    <span class="applicants-count">${opp.max_applicants ? opp.max_applicants + ' applicants' : '0 applicants'}</span>
                    <div style="display: flex; gap: 8px;">
                        <button class="view-course-btn" style="width: auto; padding: 6px 12px; background: transparent; color: var(--qf-red); border: 1px solid var(--qf-red);" onclick="deleteOpportunity(${opp.id})">Delete</button>
                        <button class="view-course-btn" style="width: auto; padding: 6px 12px; background: transparent; color: var(--qf-blue); border: 1px solid var(--qf-blue);" onclick="editOpportunity(${opp.id})">Edit</button>
                        <button class="view-course-btn" style="width: auto; padding: 6px 12px;" onclick='openOpportunityDetails("${escapeHtml(opp.name).replace(/"/g, '&quot;')}", ${JSON.stringify({
...
document.getElementById('loginForm').addEventListener('submit', async function(e) {
    ...
    // note: checkbox ID is probably 'rememberMe' based on standard UI conventions.
    const rememberMeBox = document.getElementById('rememberMe') || document.querySelector('#loginForm input[type="checkbox"]');
    const rememberMe = rememberMeBox ? rememberMeBox.checked : false;
...
document.getElementById('signupForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.errors) {
                if (data.errors.email) { showError('signupEmailErr', data.errors.email); document.getElementById('signupEmail').classList.add('error'); }
                if (data.errors.password) { showError('signupPasswordErr', data.errors.password); }
            } else {
